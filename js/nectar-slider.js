...
if ('IntersectionObserver' in window) {
  ...
  function nectarSlideRotateReset(slider, sliderAutoplayCount, interval,sliderNum){
    // $('.st-indicator-inner').stop();
    // $('.st-indicator-inner').css('width', '0%');
  ...
  function updateSlideContentOnNavBox(e) {
      //cache slider obj
      var $obj = e;

      var activeSlideObj   = $($obj.activeSlide());
      var firstSlideObj    = $($obj.getFirstSlide());
      var activeSlideIndex = $obj.activeIndex;
      var allSlides        = $obj.slides;

      //get next slide object
      var nextSlide = firstSlideObj;
      var nextIndex = activeSlideIndex + 1;
      if (nextIndex in allSlides) {
          nextSlide = $(allSlides[nextIndex]);
      }

      //add slide titles in navs (PP)
      var currSlideData = activeSlideObj.find('.slideDataItem');
      if (currSlideData) {
          //var currSlideTitle = currSlideData.data('title');
          //next slide title
          var nextSlideTitle = nextSlide.find('.slideDataItem .ns-heading-el').text();
          $($obj.container).find('.nextSlideTitle').html(nextSlideTitle);

          //desktop
          var newContent = $(currSlideData.html());
          //update title and buttons (on desktop)
          $($obj.container).find('.ppControlsContent').html("");
          $($obj.container).find('.ppControlsContent').append(newContent);
          newContent.hide().slideDown();

          //mobile
          var currSlideDataTitle   = activeSlideObj.find('.slideDataItem .ns-heading-el');
          var currSlideDataButtons = activeSlideObj.find('.slideDataItem .buttons');
          //update title
          $($obj.container).find('.ppControlsContentTitle').html(currSlideDataTitle.html());
          //update buttons
          var newContentButtons = $(currSlideDataButtons.prop('outerHTML'));
          $($obj.container).find('.ppControlsContentButtons').html("");
          $($obj.container).find('.ppControlsContentButtons').append(newContentButtons);
          newContentButtons.hide().slideDown();
      }
  }
  ...
  function onChangeStart(e){
    ...
    var swiperContainer = $($obj.container);
    var slider = swiperContainer.parent();
    var $autoplayVal = slider.attr('data-autorotate');
    if ($autoplayVal) {
      $('.st-indicator-inner').stop().animate({ width: '100%' }, parseInt($autoplayVal), function() {
        $('.st-indicator-inner').css('width', '0%');
      });
    }
    ...
    $($obj.container).find('.pp-slide-current-pos .slide-current').html( pp_pad($activeIndex2, 2) );

    if($($obj.container).attr('data-loop') === 'true'){
      //duplicate first slide
      if( $($obj.container).find('.swiper-slide-active').index()+1 == 1) { 
        $($obj.container).find('.slider-next .slide-count .slide-current, .slider-prev .slide-count .slide-current').html( $slideNum - 2 );
        $($obj.container).find('.pp-slide-current-pos .slide-current').html(pp_pad($slideNum - 2, 2));
      }
      //duplicate last slide
      else if( $($obj.container).find('.swiper-slide-active').index()+1 == $slideNum) {
        $($obj.container).find('.slider-next .slide-count .slide-current, .slider-prev .slide-count .slide-current').html( 1 );
        $($obj.container).find('.pp-slide-current-pos .slide-current').html('01');
      }
    }
    ...
    //find current slide data
    //add slide titles in navs (PP)
    updateSlideContentOnNavBox($obj);
